...
export interface RefreshResult {
  articles: {
    url: string; title: string; language: string | null; group_id: number | null
    pageviews: number; organic_users: number; revenue: number; transactions: number
...
export async function getCachedData(): Promise<RefreshResult | null> {
  const db = getDb()
